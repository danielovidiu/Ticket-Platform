...
#
#     13 and not 12 on purpose. The marker is compared for EQUALITY, not order, and the
#     open tier-id branch also claims 12 — two branches sharing a number means whichever
#     deploys second finds `current == marker`, skips setup entirely, and silently runs
#     none of its migrations. That is not hypothetical: it happened on the dev database
#     while this was being written, and the symptom was an Archive link that would not
#     go away. Whatever merges after this must take 14.
SCHEMA_VERSION = 13
