...
class TestUploadPermissions:
    ...
    # 1x1 transparent PNG, generated by Pillow and verified before being pasted here.
    #
    # The previous literal was subtly corrupt — a bad IDAT header checksum — and Pillow
    # refuses to decode it. That went unnoticed because the upload route never verified
    # the bytes: it wrote them to storage verbatim and let the thumbnail step fail into
    # an `except Exception`. So this fixture was a live demonstration of audit M8, and
    # closing M8 is what turned it red.
    PIXEL = bytes.fromhex(
