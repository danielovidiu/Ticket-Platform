...
export default function ProductDetail() {
  ...
  if (!p) return <div className="p-16 text-center text-zinc-500 font-mono-x uppercase text-xs tracking-[0.3em]">Loading…</div>;
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-6 md:px-10 py-12 grid md:grid-cols-12 gap-10">
      <div className="md:col-span-7">
        <div className="aspect-square overflow-hidden border border-white/10 bg-[#0F0F0F]">
          {images[imgIdx] ? (
            <img src={mediaUrl(images[imgIdx])} alt={p.name} className="w-full h-full object-cover" data-testid="product-image" />
          ) : (
            <div className="w-full h-full flex items-center justify-center font-mono-x text-xs uppercase tracking-[0.3em] text-zinc-600">No image</div>
          )}
        </div>
        {images.length > 1 && (
          <div className="mt-2 flex gap-2">
            {images.map((src, i) => (
              <button key={src + i} onClick={() => setImgIdx(i)}
                      className={`w-20 h-20 overflow-hidden border ${i === imgIdx ? "border-white" : "border-white/10"}`}>
      ...
      <div className="md:col-span-5">
        <div className="border border-white/10 bg-[#0F0F0F] p-6 md:p-8 sticky top-24">
          <div className="font-mono-x text-xs uppercase tracking-[0.3em] text-zinc-500">
          ...
          <div className="font-mono-x text-[10px] uppercase tracking-[0.2em] text-zinc-500 mt-1">VAT included</div>

          {p.description && <p className="mt-6 text-zinc-300 text-sm leading-relaxed">{p.description}</p>}

          <div className="mt-8">
            <div className="font-mono-x text-[10px] uppercase tracking-[0.2em] text-zinc-500 mb-2">Size</div>
              ...
              {p.variants.map((v) => (
                <button
                  ...
                  className={`px-4 py-2 border font-mono-x text-xs uppercase tracking-[0.15em] transition-colors ${
                    v.variant_id === variantId ? "bg-white text-black border-white"
                      : v.in_stock ? "border-white/25 text-zinc-200 hover:border-white"
                      : "border-white/10 text-zinc-600 line-through cursor-not-allowed"}`}>
                  {v.size || "One size"}
                </button>
              ))}
            </div>
            {variant && <div className="font-mono-x text-[10px] uppercase tracking-[0.2em] text-zinc-600 mt-2">SKU {variant.sku}</div>}
          </div>
          ...
          <div className="mt-4 font-mono-x text-[10px] uppercase tracking-[0.2em] text-zinc-500 leading-relaxed">
